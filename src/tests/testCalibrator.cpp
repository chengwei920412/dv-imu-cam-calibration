...
template<typename G, typename D>
void pushImu(Calibrator<G, D>& calibrator, const std::vector<fs::path>& imuPaths) {
...
TEST(CalibratorNoImuTest, EquidistantDistortionModel) {
    ...
    pushImages<aslam::cameras::EquidistantDistortedPinholeCameraGeometry, aslam::cameras::EquidistantDistortion>(
        calibrator,
        imgPaths);
...
TEST(CalibratorNoImuTest, RadTanDistortionModel) {
    ...
    pushImages<aslam::cameras::DistortedPinholeCameraGeometry, aslam::cameras::RadialTangentialDistortion>(
        calibrator,
        imgPaths);
...
TEST(CalibratorTest, EquidistantDistortionModel) {
    ...
    pushImu<aslam::cameras::EquidistantDistortedPinholeCameraGeometry, aslam::cameras::EquidistantDistortion>(
        calibrator,
        imuPaths);
...
TEST(CalibratorTest, RadTanDistortionModel) {
    ...
    pushImu<aslam::cameras::DistortedPinholeCameraGeometry, aslam::cameras::RadialTangentialDistortion>(
        calibrator,
        imuPaths);
    ...
    try {
        ...
        expected << 0.822953, -0.566575, -0.0417222, -1.47794, 0.561684, 0.822462, -0.0898166, 1.75718, 0.0852028,
            0.0504802, 0.995084, -7.997, 0, 0, 0, 1;
        EXPECT_TRUE(result.T_cam_imu.isApprox(expected, 1e-3)) << "result:" << std::endl
