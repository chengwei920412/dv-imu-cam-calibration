...
CameraGeometry::CameraGeometry(boost::shared_ptr<IccCamera> camera) {
    iccCamera = camera;

    auto geometry = camera->getCameraGeometry();

    auto casted = boost::dynamic_pointer_cast<aslam::cameras::DistortedPinholeCameraGeometry>(geometry);
    if (casted == nullptr) {
        throw std::runtime_error("Something went wrong with casting of the pointer of camera geometry");
    }

    // create the design variables
    dv = boost::make_shared<aslam::backend::CameraDesignVariable<aslam::cameras::DistortedPinholeCameraGeometry>>(
        casted);
...
bool CameraGeometry::calibrateIntrinsics(
    ...
    typedef aslam::backend::ReprojectionError<aslam::cameras::DistortedPinholeCameraGeometry> ReprojectionError;
...
CalibrationTargetOptimizationProblem::CalibrationTargetOptimizationProblem(
    ...
    for (size_t i = 0; i < P_t_ex.size(); ++i) {
        ...
        if (rig_observation.imagePoint(i, y)) {
            ...
            auto rerr
                = boost::make_shared<aslam::backend::ReprojectionError<aslam::cameras::DistortedPinholeCameraGeometry>>(
                    y,
                    invR,
...
bool CameraCalibration::addTargetView(const aslam::cameras::GridCalibrationTargetObservation& observation, bool force) {
    auto T_tc_guess = observation.T_t_c();
    auto batch_problem = boost::make_shared<CalibrationTargetOptimizationProblem>(
        camera,
        target,
        T_tc_guess,
        observation,
...
size_t CameraCalibration::getNumBatches() {
    return estimator->getNumBatches();
}

void CameraCalibration::replaceBatch(
    const size_t batch_id,
    boost::shared_ptr<CalibrationTargetOptimizationProblem> new_batch) {
...
std::tuple<
    ...
    std::vector<std::vector<Eigen::MatrixXd>>,
    std::vector<std::vector<Eigen::MatrixXd>>>
    CameraCalibration::getReprojectionErrors(const size_t cameraId) {
...
CameraCalibration::CalibrationResult CameraCalibration::getResult() {
    ...
    ErrorInfo err_info(Eigen::Vector2d(-1., -1.), Eigen::Vector2d(-1., -1.));
    const auto [corners, reprojs, rerrs] = getReprojectionErrors(0);
    if (!rerrs.empty()) {
        const auto [me, std] = getReprojectionErrorStatistics(rerrs);
        err_info = ErrorInfo(me, std);
    }

    return CameraCalibration::CalibrationResult(projection, distortion, err_info);
}

void CameraCalibration::printResult(const CalibrationResult& result, std::stringstream& ss) {
...
boost::shared_ptr<CalibrationTargetOptimizationProblem> CameraCalibration::removeCornersFromBatch(
    const size_t batch_id,
    ...
    // rebuild problem
    auto new_problem = boost::make_shared<CalibrationTargetOptimizationProblem>(
        batch->camera,
        batch->target,
        batch->T_tc_guess,
